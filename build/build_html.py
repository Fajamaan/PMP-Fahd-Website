...
.utilbar{display:flex;justify-content:flex-end;margin:0 0 10px}
...
<div class="wrap">
  <div id="quizscreen">
  <div class="utilbar"><button id="overviewbtn" class="btn ghost">📋 Question list / overview</button></div>
  <div class="tabs">
...
function render(){
  ...
  if(bkmktog) bkmktog.addEventListener('click',()=>{ toggleBookmark(q); render(); });
...
function renderOverview(){
  ...
  const sc=$('#startcustom'); if(sc) sc.onclick=()=> startCustomSet(rows.map(gi=>list[gi]));
}
...
function startCustomSet(qs){
  if(!qs.length) return;
  SETS.custom.list = qs.slice();
  view.custom = {idx:0,state:{}};
  cur='custom';
  customLabel = (ovBank==='orig'?'Original':'New set')+' · '+(FLABEL[ovFilter]||ovFilter);
