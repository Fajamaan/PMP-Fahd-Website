...
.pkgdone{margin:0 0 14px;padding:12px 14px;border-radius:12px;border:1px solid var(--green);background:var(--greenbg);color:#dfffe8;font-size:14px;align-items:center;gap:12px;flex-wrap:wrap}
.pkgdone b{color:#fff}
.pkgdone .grow{flex:1;min-width:200px}
.footer-note{color:var(--muted);font-size:12px;text-align:center;margin-top:24px;line-height:1.6}
.checkbtn{margin-top:12px}
.utilbar{display:flex;justify-content:flex-end;gap:8px;margin:0 0 10px;flex-wrap:wrap}
.badge2{background:#2a325f;color:#ffd166;border-radius:999px;padding:1px 8px;font-size:12px;margin-left:4px}
.toggle.bkmk.on{color:#ffd166}
.custominfo{margin:0 0 12px;padding:10px 14px;border-radius:12px;border:1px solid var(--accent);background:#1a2150;color:#dfe4ff;font-size:13.5px;align-items:center;gap:12px;flex-wrap:wrap}
.custominfo .grow{flex:1;min-width:160px}
.custominfo b{color:#fff}
.ov-head{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px}
.ov-head h2{font-size:18px;margin:0}
.ov-banktabs{display:flex;gap:8px;margin-bottom:12px}
.ov-banktabs button{flex:1;background:var(--card);border:1px solid var(--border);color:var(--muted);border-radius:10px;padding:9px;font-size:13.5px;font-weight:600;cursor:pointer}
.ov-banktabs button.on{background:linear-gradient(90deg,#26305c,#2f3a6e);color:var(--text);border-color:var(--accent)}
.ov-filters{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}
.chip{background:var(--card2);border:1px solid var(--border);color:var(--muted);border-radius:999px;padding:7px 12px;font-size:12.5px;cursor:pointer;user-select:none}
.chip.on{background:var(--accent);color:#fff;border-color:var(--accent)}
.ov-actions{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:12px}
.ov-actions .cnt{color:var(--muted);font-size:13px}
.ov-list{display:flex;flex-direction:column;gap:6px;max-height:60vh;overflow:auto;padding-right:4px}
.ov-row{display:flex;gap:10px;align-items:center;background:var(--card2);border:1px solid var(--border);border-radius:10px;padding:9px 12px;cursor:pointer;transition:.12s}
.ov-row:hover{border-color:var(--accent);background:#1b2147}
.ov-row .num{font-weight:800;color:var(--accent);min-width:42px;font-size:13px}
.ov-row .qt{flex:1;font-size:13px;color:#cfd6f7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.ov-row .st{display:flex;gap:4px;align-items:center}
.dot{font-size:13px;width:22px;text-align:center}
.dot.green{color:var(--green)}.dot.red{color:var(--red)}.dot.muted{color:var(--muted)}
@media(max-width:560px){.qtext{font-size:16px}.opt{font-size:14px}.ov-row .qt{font-size:12px}}
</style>
</head>
<body>
<header>
...
<div class="wrap">
  <div id="quizscreen">
  <div class="utilbar">
    <button id="studybkmk" class="btn ghost">🔖 Study bookmarks</button>
    <button id="overviewbtn" class="btn ghost">📋 List <span id="bkmkbadge" class="badge2">🔖 0</span></button>
  </div>
  <div class="tabs">
  ...
  <div id="pkgdone" class="pkgdone" style="display:none"></div>
  ...
  <div id="custominfo" class="custominfo" style="display:none"></div>
  ...
  <div id="overviewscreen" style="display:none"></div>
</div>
...
const SETS = { orig:{list:ORIG}, news:{list:NEWS}, wrong:{list:[]}, custom:{list:[]} };
const view = { orig:{idx:0,state:{},pkg:0}, news:{idx:0,state:{},pkg:0}, wrong:{idx:0,state:{}}, custom:{idx:0,state:{}} };
let cur = 'orig';
...
function buildPkgSelector(){
  ...
  if(cur!=='orig' && cur!=='news'){ if(bar) bar.style.display='none'; return; }   // only banks are packaged
  ...
  for(let i=0;i<n;i++){
    ...
    const size=b-a+1, done=pkgAnswered(cur,i);
    const tag = done>=size ? ' ✓ done' : (done>0 ? ' · '+done+'/'+size : '');
    o.value=i; o.textContent='Package '+(i+1)+' of '+n+' · Q'+a+'–'+b+tag;
    sel.appendChild(o);
  }
...
// --- Persistent per-question results: powers resume, package completion + ✓ markers ---
let results;
try{ results = JSON.parse(localStorage.getItem('pmpResV1')||'{}') || {}; }catch(e){ results = {}; }
function saveResults(){ try{ localStorage.setItem('pmpResV1', JSON.stringify(results)); }catch(e){} }
function pkgAnswered(set, i){
  const a=i*PKG, b=Math.min((i+1)*PKG, BANKS[set].length);
  let c=0; for(let j=a;j<b;j++){ if(results[qSig(BANKS[set][j])]) c++; }
  return c;
}

// --- Bookmarks: flag questions to revisit later ---
let bookmarks;
try{ bookmarks = new Set(JSON.parse(localStorage.getItem('pmpBookmV1')||'[]')); }catch(e){ bookmarks = new Set(); }
bookmarks = new Set([...bookmarks].filter(k=>_allSigs.has(k)));
function saveBookmarks(){ try{ localStorage.setItem('pmpBookmV1', JSON.stringify([...bookmarks])); }catch(e){} }
function isBookmarked(q){ return bookmarks.has(qSig(q)); }
function toggleBookmark(q){ const k=qSig(q); if(bookmarks.has(k)) bookmarks.delete(k); else bookmarks.add(k); saveBookmarks(); }
function qStatus(q){ const r=results[qSig(q)]; return { answered:!!r, correct:r?r.c:null, bookmarked:bookmarks.has(qSig(q)) }; }
...
function render(){
  ...
  const rtl = q.lang==='ar';
  const sig = qSig(q);
  const saved = results[sig];
  const st = saved
    ? {picked:new Set(saved.p), done:true, correct:saved.c}
    : (state[idx] || (state[idx] = {picked:new Set(), done:false, correct:false}));
  ...
  const arTog = (fromOrig && q.aq)
    ? `<span class="toggle" id="artog">🌐 العربية</span>` : '';
  const bkmkTog = `<span class="toggle bkmk ${isBookmarked(q)?'on':''}" id="bkmktog">${isBookmarked(q)?'🔖 Saved':'🔖 Save'}</span>`;

  card.innerHTML = `
    <div class="qmeta">
      <span>${metaLeft}</span>
      <span style="display:flex;gap:12px;align-items:center">${bkmkTog}${arTog}</span>
    </div>
  ...
  const bkmktog = document.getElementById('bkmktog');
  if(bkmktog) bkmktog.addEventListener('click',()=>{ toggleBookmark(q); updateBookmarkUI(); render(); });
  ...
  function commit(){
    ...
    results[sig] = {c:st.correct, p:pickedArr};
    saveResults();
    markResult(q, st.correct);
    saveLast();
    buildPkgSelector();   // refresh ✓ / progress in the dropdown
    render();
  }
  function wireClaude(){
...
function updateBar(){
  ...
  let s=0,a=0;
  for(const qq of S.list){ const rr=results[qSig(qq)]; if(rr){a++; if(rr.c)s++;} }
  $('#score').textContent=s; $('#answered').textContent=a;
  const wc=$('#wrongctl'); if(wc) wc.style.display = (cur==='wrong' && wrongKeys.size) ? 'block':'none';
  renderCustomInfo(N, s);
  renderPkgBanner(N, s, a);
}

function renderCustomInfo(N, s){
  const el=$('#custominfo'); if(!el) return;
  if(cur!=='custom'){ el.style.display='none'; return; }
  el.innerHTML = '<span class="grow">🎯 <b>Custom set</b> — '+N+' question'+(N===1?'':'s')+' ('+customLabel+'). Score '+s+'/'+N+'.</span>'
    + '<button class="btn ghost" id="backtolist" style="padding:6px 12px;font-size:13px">📋 Back to list</button>';
  el.style.display='flex';
  const b=$('#backtolist'); if(b) b.onclick=showOverview;
}

function renderPkgBanner(N, s, a){
  const el=$('#pkgdone'); if(!el) return;
  if((cur!=='orig' && cur!=='news') || N===0 || a<N){ el.style.display='none'; return; }   // only packaged banks, fully answered
  const pct=Math.round(s/N*100), p=view[cur].pkg, hasNext = p < pkgCount(cur)-1;
  el.innerHTML = '<span class="grow">🎉 <b>Package '+(p+1)+' complete!</b> You scored <b>'+s+' / '+N+'</b> correct ('+pct+'%).</span>'
    + '<button class="btn ghost" id="retakepkg" style="padding:7px 12px;font-size:13px">↻ Retake</button>'
    + (hasNext ? '<button class="btn" id="nextpkg" style="padding:7px 12px;font-size:13px">Next package →</button>' : '');
  el.style.display='flex';
  const rt=$('#retakepkg'); if(rt) rt.onclick=()=>{
    for(const qq of SETS[cur].list){ delete results[qSig(qq)]; }
    saveResults(); view[cur].idx=0; view[cur].state={}; buildPkgSelector(); render();
  };
  const np=$('#nextpkg'); if(np) np.onclick=()=>{
    view[cur].pkg += 1; view[cur].idx=0; view[cur].state={};
    applyPackage(cur); saveLast(); buildPkgSelector(); render();
  };
}
...
// --- Question Overview sub-page: status list, filters, bookmarks, custom sets ---
let ovBank = 'orig';     // which bank the overview lists
let ovFilter = 'all';    // all | unanswered | answered | correct | wrong | bookmarked
let customLabel = '';    // description of the active custom set
const FILTERS = [
  ['all','All'], ['unanswered','⬜ Unanswered'], ['answered','📝 Answered'],
  ['correct','✔️ Correct'], ['wrong','✖️ Wrong'], ['bookmarked','🔖 Bookmarked']
];
const FLABEL = {all:'all questions',unanswered:'unanswered',answered:'answered',correct:'correct',wrong:'wrong',bookmarked:'bookmarked'};
function matchesFilter(q, f){
  const st = qStatus(q);
  if(f==='answered') return st.answered;
  if(f==='unanswered') return !st.answered;
  if(f==='correct') return st.answered && st.correct===true;
  if(f==='wrong') return st.answered && st.correct===false;
  if(f==='bookmarked') return st.bookmarked;
  return true;
}
function esc(s){ return String(s).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c])); }
function showOverview(){
  if(cur==='orig'||cur==='news') ovBank=cur;
...
function renderOverview(){
  const scr=$('#overviewscreen'), list=BANKS[ovBank];
  const rows=[];
  for(let i=0;i<list.length;i++){ if(matchesFilter(list[i], ovFilter)) rows.push(i); }
  let html = '<div class="ov-head"><h2>📋 Question overview</h2>'
    + '<button class="btn ghost" id="ovback" style="padding:7px 12px;font-size:13px">← Back to quiz</button></div>'
    + '<div class="ov-banktabs">'
    + '<button data-b="orig" class="'+(ovBank==='orig'?'on':'')+'">📘 Original ('+ORIG.length+')</button>'
    + '<button data-b="news" class="'+(ovBank==='news'?'on':'')+'">🆕 New set ('+NEWS.length+')</button>'
    + '</div>'
    + '<div class="ov-filters">'+FILTERS.map(f=>'<span class="chip '+(ovFilter===f[0]?'on':'')+'" data-f="'+f[0]+'">'+f[1]+'</span>').join('')+'</div>'
    + '<div class="ov-actions"><span class="cnt">Showing <b>'+rows.length+'</b> of '+list.length+'</span>'
    + (rows.length ? '<button class="btn" id="startcustom" style="padding:8px 14px;font-size:13px">▶ Start set with these ('+rows.length+')</button>' : '')
    + '</div><div class="ov-list">';
  for(const gi of rows){
    const q=list[gi], st=qStatus(q);
    const ansDot = st.answered
      ? (st.correct?'<span class="dot green" title="Correct">✔️</span>':'<span class="dot red" title="Wrong">✖️</span>')
      : '<span class="dot muted" title="Not answered">⬜</span>';
    const bm = st.bookmarked ? '<span class="dot" title="Bookmarked">🔖</span>' : '<span class="dot muted">·</span>';
    html += '<div class="ov-row" data-gi="'+gi+'"><span class="num">#'+(gi+1)+'</span>'
      + '<span class="qt">'+esc(q.q)+'</span><span class="st">'+ansDot+bm+'</span></div>';
  }
  html += '</div>';
  if(!rows.length) html += '<p class="hint" style="text-align:center;margin-top:20px">No questions match this filter.</p>';
  scr.innerHTML = html;
  $('#ovback').onclick = showQuiz;
  scr.querySelectorAll('.ov-banktabs button').forEach(b=> b.onclick=()=>{ ovBank=b.dataset.b; renderOverview(); });
  scr.querySelectorAll('.chip').forEach(c=> c.onclick=()=>{ ovFilter=c.dataset.f; renderOverview(); });
  scr.querySelectorAll('.ov-row').forEach(r=> r.onclick=()=> jumpToGlobal(ovBank, parseInt(r.dataset.gi,10)) );
  const sc=$('#startcustom'); if(sc) sc.onclick=()=> startCustomSet(rows.map(gi=>list[gi]), (ovBank==='orig'?'Original':'New set')+' · '+(FLABEL[ovFilter]||ovFilter));
}
function jumpToGlobal(bank, gi){
  cur=bank;
  view[bank].pkg = Math.floor(gi/PKG);
  applyPackage(bank);
  view[bank].idx = gi % PKG;
  document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active', t.dataset.set===bank));
  buildPkgSelector(); saveLast(); showQuiz(); render();
}
function startCustomSet(qs, label){
  if(!qs.length) return;
  SETS.custom.list = qs.slice();
  view.custom = {idx:0,state:{}};
  cur='custom';
  customLabel = label || 'custom selection';
  document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));
  buildPkgSelector(); showQuiz(); render();
}
function studyBookmarks(){
  const qs = ALL.filter(q=>isBookmarked(q));
  if(!qs.length){ alert('No bookmarked questions yet.\nTap “🔖 Save” on any question to add it here.'); return; }
  startCustomSet(qs, 'your bookmarks');
}
function updateBookmarkUI(){
  const b=$('#bkmkbadge'); if(b) b.textContent='🔖 '+bookmarks.size;
  const sb=$('#studybkmk'); if(sb){ sb.style.opacity = bookmarks.size? '1':'0.5'; sb.title = bookmarks.size? ('Start a set of your '+bookmarks.size+' bookmarked question'+(bookmarks.size===1?'':'s')) : 'No bookmarks yet'; }
}
$('#overviewbtn').onclick = showOverview;
$('#studybkmk').onclick = studyBookmarks;
updateBookmarkUI();

// --- Initialize packages, then restore the last session ---
// On reopen, skip questions already answered and land on the first one not yet done.
function resumeIdx(set){
  const lst = SETS[set].list;
  for(let i=0;i<lst.length;i++){ if(!results[qSig(lst[i])]) return i; }
  return Math.max(0, lst.length-1);   // whole package already answered -> last question (completion banner shows)
}
(function restoreLast(){
  ...
  view.orig.idx = resumeIdx('orig');   // resume where you stopped, past answered questions
  view.news.idx = resumeIdx('news');
})();
