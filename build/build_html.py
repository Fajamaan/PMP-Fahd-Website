...
<header>
  ...
  <span class="sub">Two question banks · pick an answer to see the correct one + an explanation</span>
</header>
<div class="wrap">
  <div class="tabs">
    <button class="tab active" data-set="orig">📘 Original deck · <b id="cnt-orig">0</b></button>
...
const SETS = { orig:{list:ORIG}, news:{list:NEWS} };
const view = { orig:{idx:0,state:{}}, news:{idx:0,state:{}} };
let cur = 'orig';
...
function render(){
  ...
  if(N===0){
    card.innerHTML = `<p class="qtext">No questions in this set yet.</p>
  ...
  card.innerHTML = `
    ...
    ${(cur==='orig' && q.aq) ? `<div class="ar" id="arbox">
        <div><b>${q.aq}</b></div>
...
$('#prev').onclick=()=>{ const V=view[cur]; if(V.idx>0){V.idx--;render();} };
$('#next').onclick=()=>{ const V=view[cur]; if(V.idx<SETS[cur].list.length-1){V.idx++;render();} };
$('#jumpbtn').onclick=()=>{
  const N=SETS[cur].list.length; let v=parseInt($('#jumpin').value,10);
  if(v>=1&&v<=N){view[cur].idx=v-1;render();}
};
